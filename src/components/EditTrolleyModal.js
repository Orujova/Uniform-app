...
const ModalContent = styled.div`
  ...
  width: 100%; max-width: 650px; /* Genişlik ayarlanabilir */
...
const ModalBody = styled.div`
  padding: 1.25rem 1.5rem; overflow-y: auto; flex-grow: 1; max-height: calc(80vh - 110px);
  display: flex; flex-direction: column; gap: 1rem;
`;

// --- İki Sütunlu Yapı ---
const TopSectionContainer = styled.div`
  display: flex; gap: 1.5rem; width: 100%; border-bottom: 1px solid #e5e7eb;
...
const Form = styled.form` // Sadece sayım ve butonlar
  display: flex; flex-direction: column; gap: 1.2rem; // Gap biraz artırıldı
`;
const FormGroup = styled.div` display: flex; flex-direction: column; gap: 0.4rem; `;
const Label = styled.label` font-weight: 500; color: #374151; font-size: 0.85rem; display: flex; align-items: center; gap: 0.3rem; margin-bottom: 2px; `;
...
const ReadOnlyInfo = styled.div` // Sabit bilgiyi gösteren stil
  ${sharedInputStyles} background-color: #f3f4f6; color: #4b5563;
  cursor: default; display: flex; align-items: center; gap: 0.4rem; font-weight: 500;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis; // Taşmayı önle
  &:focus { border-color: #d1d5db; box-shadow: none; }
`;
const CountInputGroup = styled.div` display: grid; grid-template-columns: 1fr 1fr; gap: 0.8rem; `;

const LargeTypeImagePreview = styled.div` // CreateModal'daki ile benzer, boyut ayarlanabilir
  width: 100%; // Sağ sütunu doldur
  max-width: 110px; // Edit için biraz daha küçük
  aspect-ratio: 1 / 1; height: auto; border-radius: 10px; border: 1px solid #e5e7eb;
  background-color: #f8f9fa; display: flex; align-items: center; justify-content: center;
  overflow: hidden; color: #cbd5e1; position: relative; margin: 0 auto;

  svg { font-size: 2.5rem; } // İkon boyutu
...
`;
const PreviewLabel = styled(Label)` // Resim altı etiket
    font-size: 0.75rem; justify-content: center; margin-top: 0.4rem;
    color: #6b7280; text-align: center; &.error { color: #ef4444; font-weight: 600;}
`;
...
// --- Component Logic ---
const EditTrolleyModal = ({ isOpen, onClose, onSaveSuccess, initialData, apiBaseUrl = API_BASE_URL, token, }) => {
  ...
  const fetchTrolleyTypeImage = useCallback(async (typeId) => {
    if (!typeId || !token) { setImageUrl(null); return; }
    setIsImageLoading(true); setImageLoadError(false); setImageUrl(null);
    try {
      const response = await fetch(`${apiBaseUrl}/api/TrolleyType/${typeId}`, { method: 'GET', headers: { 'Authorization': `Bearer ${token}`, 'Accept': 'application/json' }});
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const typeData = await response.json();
      const displayUrl = generateDisplayImageUrl(typeData.ImageUrl);
      setImageUrl(displayUrl);
    } catch (err) { console.error("Err fetch img:", err); setImageUrl(null); /* img onError halledecek */ }
  ...
  const handleSubmit = async (event) => {
    ...
    if (isNaN(working) || working < 0 || isNaN(broken) || broken < 0) { setError("Counts must be zero or positive numbers."); showToast("Counts must be zero or positive numbers.", "error"); return; }
    if (initialData.Id == null || initialData.ProjectId == null || initialData.TrolleyTypeId == null) { setError("Missing necessary IDs to update."); showToast("Missing IDs.", "error"); return; }
    setIsLoading(true); setError("");
    const queryParams = new URLSearchParams({ Id: initialData.Id.toString(), ProjectId: initialData.ProjectId.toString(), TrolleyTypeId: initialData.TrolleyTypeId.toString(), WorkingTrolleysCount: working.toString(), BrokenTrolleysCount: broken.toString()});
    const fetchUrl = `${apiBaseUrl}/api/Trolley?${queryParams.toString()}`;
    try {
        const response = await fetch(fetchUrl, { method: "PUT", headers: { Authorization: `Bearer ${token}` }});
        if (!response.ok) { let errorMsg = `HTTP Error: ${response.status}`; try { const eData = await response.json(); errorMsg = eData.Message || eData.title || errorMsg; } catch (e) {/*ignore*/} throw new Error(errorMsg); }
        showToast("Trolley counts updated!", "success");
        onSaveSuccess(); onClose();
    } catch (err) { console.error("Update Error:", err); const errMsg = err.message || "Update error."; setError(errMsg); showToast(errMsg, "error"); }
  ...
  return (
    <ModalOverlay onClick={onClose}>
      <ModalContent onClick={(e) => e.stopPropagation()}>
        <ModalHeader> <Title> <FaEdit /> Edit Counts </Title> <CloseButton onClick={onClose} disabled={isLoading}> <FaTimes size={16} /> </CloseButton> </ModalHeader>
        ...
        <ModalBody>
          ...
          <TopSectionContainer>
            ...
            <RightColumn>
                <LargeTypeImagePreview
                     className={`${imageLoadError ? 'error' : ''} ${isImageLoading ? 'loading' : ''}`}
                     title={isImageLoading ? "Loading image..." : (imageLoadError ? "Image load error" : (imageUrl ? (initialData.TrolleyTypeName || 'Trolley Type') : "No image found"))}>
                       {isImageLoading ? ( <PreviewSpinnerWrapper><StyledSpinner size={32}/></PreviewSpinnerWrapper> )
                       : imageUrl && !imageLoadError ? ( <img src={imageUrl} alt={initialData.TrolleyTypeName || 'Trolley type'} onError={() => setImageLoadError(true)}/> )
                       : ( imageLoadError ? <FaExclamationTriangle/> : <FaImage/> )}
                </LargeTypeImagePreview>
                {/* Opsiyonel etiket */}
                {initialData.TrolleyTypeName && !isImageLoading && <PreviewLabel className={imageLoadError ? 'error' : ''}>{imageLoadError ? 'Error' : ''}</PreviewLabel>}
          ...
          <Form id="edit-trolley-count-form" onSubmit={handleSubmit} noValidate>
              <CountInputGroup>
                  <FormGroup>
                      <Label> <FaShoppingCart color="#16a34a" /> Working </Label>
                      <Input type="number" value={workingCount} onChange={(e) => setWorkingCount(e.target.value)} placeholder="Count" min="0" required disabled={isActionDisabled} aria-label="Working Trolley Count"/>
                  </FormGroup>
                  <FormGroup>
                      <Label> <FaShoppingCart color="#dc2626" /> Broken </Label>
                       <Input type="number" value={brokenCount} onChange={(e) => setBrokenCount(e.target.value)} placeholder="Count" min="0" required disabled={isActionDisabled} aria-label="Broken Trolley Count"/>
                  </FormGroup>
              </CountInputGroup>
          </Form>
        ...
        <ModalFooter>
          ...
          <PrimaryButton type="submit" form="edit-trolley-count-form" disabled={isActionDisabled}>
