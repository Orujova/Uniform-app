...
const TrolleyWorkDaysModal = ({ isOpen, onClose }) => {
  ...
  const updateWeekDay = async (id) => {
    try {
      ...
      await fetch(`${API_BASE_URL}/api/UniformForEmployee/update-week-trolleycountingallowday`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
        body: JSON.stringify({ TrolleyCountingAllowDay: selectedDayObj.TrolleyCountingAllowDay }),
      });
  ...
  return (
    <ModalOverlay onClick={onClose}>
      <ModalContent onClick={(e) => e.stopPropagation()}>
        <ModalHeader>
          <ModalTitle>Trolley Counting Days Management</ModalTitle>
        ...
        <WeekDayContainer>
          {weekDays.map((day) => (
            <WeekDayButton
              key={day.Id}
              isActive={day.TrolleyCountingIsActive}
              $isSelected={selectedDay === day.Id}
              onClick={() => setSelectedDay(day.Id)}
              disabled={loading}
            >
              <WeekDayText>{day.TrolleyCountingAllowDay}</WeekDayText>
              <StatusIcon>
                {day.TrolleyCountingIsActive ? (
                  <>
                    <FaCheck size={16} />
                    Active
                  </>
...
export default TrolleyWorkDaysModal;